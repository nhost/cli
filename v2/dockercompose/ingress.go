...
func (i Ingress) Labels() map[string]string {
	labels := map[string]string{
		...
		fmt.Sprintf("traefik.http.routers.%s.tls", i.Name):                       fmt.Sprintf("%t", i.TLS),
		fmt.Sprintf("traefik.http.services.%s.loadbalancer.server.port", i.Name): fmt.Sprintf("%d", i.Port),
	}
	...
	if i.Rewrite != nil {
		...
		labels[fmt.Sprintf("traefik.http.routers.%s.middlewares", i.Name)] = fmt.Sprintf("replace-%s", i.Name)
	}
