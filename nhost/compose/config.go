...
func (c Config) functionsServiceEnvs() env {
	...
	e.merge(env{
		...
		"NHOST_REGION":         "",
		"NHOST_HASURA_URL":     fmt.Sprintf("http://traefik:%d", proxyPort),
		"NHOST_GRAPHQL_URL":    fmt.Sprintf("http://traefik:%d/v1/graphql", proxyPort),
		"NHOST_AUTH_URL":       fmt.Sprintf("http://traefik:%d/v1/auth", proxyPort),
		"NHOST_STORAGE_URL":    fmt.Sprintf("http://traefik:%d/v1/storage", proxyPort),
		"NHOST_FUNCTIONS_URL":  fmt.Sprintf("http://traefik:%d/v1/functions", proxyPort),
